    ...
    class VisitorIntelligenceTracker {
        ...
        _persistInlineInjection(intent, uniqueId, targetSelector, position, htmlPayload, scopedCss, jsPayload) {
            try {
                ...
                // Remove any prior entry for same uniqueId (dedup) and expired entries
                const TTL_MS = 24 * 60 * 60 * 1000; // 24 hours
                entries = entries.filter(e => e.uniqueId !== uniqueId && (Date.now() - e.timestamp) < TTL_MS);
        ...
        _handleInlineInject(uniqueId, targetSelector, injectionPosition, htmlPayload, scopedCss, jsPayload, priority, intent, originalId) {
            ...
            if (intent && !uniqueId.endsWith('_r')) {
                console.log(`[Tracker] [InlineInject] Persisting injection to localStorage: intent=${intent}, uniqueId=${uniqueId}`);
                this._persistInlineInjection(intent, uniqueId, targetSelector, injectionPosition, htmlPayload, scopedCss, jsPayload);
            } else {
                console.log(`[Tracker] [InlineInject] NOT persisting: intent=${intent || '(empty)'}, isRestore=${uniqueId.endsWith('_r')}`);
            }
